...
| GAi_intrin | Mg-switch sigmoid scale, slope and sensitivity | `zeros(m,1)` | Confusing notation, but this parameter refers to NMDA non-linearity. Actually, it is unclear why it has m rows, when only the first three values are used (scale, slope, sensitivity).| spm_fx_cmm_NMDA.m |
